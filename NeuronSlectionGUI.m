function varargout = NeuronSlectionGUI(varargin)
...
% Returns an image RGB (N x M X 3)
    ...
    I_mask = zeros(size(I, 1) + 1000, size(I, 2) + 1000, 'logical');
    I_sel = I_mask;
    ...
    for k = 1:size(P, 1)
        ...
        R = P(k, 3);
        if k == S
            I_sel(Y - R : Y + R, X - R : X + R) = ...
                I_sel(Y - R : Y + R, X - R : X + R) | strel('disk', R + 1).Neighborhood;
        else
            I_mask(Y - R : Y + R, X - R : X + R) = ...
                I_mask(Y - R : Y + R, X - R : X + R) | strel('disk', R + 1).Neighborhood;
        end
    end
    ...
    RGB(:, :, 1) = uint8(0.6 * 204 * I_mask + 0.6 * 255 * I_sel + I);
    RGB(:, :, 2) = uint8(0.6 * 255 * I_mask + I);
    RGB(:, :, 3) = uint8(0.6 * 204 * I_sel + I);
...
if p(1) > ref(1) && p(1) <= ref(1) + ref(3) && p(2) > ref(2) && p(2) <= ref(2) + ref(4)
    ...
    if isempty(P)
        ...
        [v, idx] = min(sqrt((P(:, 1) - c).^2 + (P(:, 2) - r).^2));
    end
    ...
    handles.ColoredImage{k} = highlightImage(handles.Image(:, :, k), P, idx_selected);
...
if ~isempty(handles.TableLabel.UserData) && handles.TableLabel.UserData(1) ~= k
    ...
    handles.TableLabel.UserData = [];
    handles.ColoredImage{k_old} = highlightImage(handles.Image(:, :, k_old), P, 0);
end
